...
export const Lens = ({
  ...
  const [zoomFactor, setZoomFactor] = useState(
    Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, initialZoomFactor))
  );
  ...
  const handleWheel = (e) => {
    if (e.deltaY < 0) {
      setZoomFactor((prev) => Math.min(MAX_ZOOM, prev + ZOOM_STEP));
    } else {
      setZoomFactor((prev) => Math.max(MIN_ZOOM, prev - ZOOM_STEP));
    }
  };
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative overflow-hidden z-20"
      onMouseEnter={() => {
        setIsHovering(true);
      }}
